...
async function apiFetch(url: string, init?: RequestInit): Promise<Response> {
  try {
    return await fetch(url, { ...defaultFetchInit, ...init });
  } catch {
    throw new Error(
...
export async function logoutUser() {
  const response = await apiFetch(apiUrl("/api/logout"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
...
export async function fetchFeedbacks() {
  const response = await apiFetch(apiUrl("/api/dashboard/feedbacks"), {
    headers: { "Content-Type": "application/json" },
  });
...
export async function fetchAiSummary() {
  const response = await apiFetch(apiUrl("/api/dashboard/summary"), {
    headers: { "Content-Type": "application/json" },
  });
